# wasivst Architecture

## Overview

wasivst runs unmodified Windows VST2/VST3 plugin DLLs in the browser by
stacking four layers:

```
Browser AudioWorklet thread
  └─ wasivst-qemu.wasm  (QEMU x86-64 system emulator, compiled by Emscripten)
       └─ Linux guest rootfs (Alpine + Wine, packed as rootfs.ext4)
            └─ wasivst-host  (headless VST2/VST3 host binary, runs under Wine)
                 └─ MyPlugin.vst3 or MyPlugin.dll  (unmodified Windows plugin)
```
...
```
Main thread JS
  │  WasiVST.load()      → loads plugin path into guest via virtio-serial
  │  setParam/getParam   → MessageChannel → worklet → serial
  │
AudioWorklet thread (wasivst-worklet.js)
  │  process()  → writes Float32 audio into SharedArrayBuffer
  │            ← reads Float32 audio out of SharedArrayBuffer
  │  serial channel → QEMU virtio-serial → wasivst-host stdin
  │
QEMU guest (wasivst-host under Wine)
  │  reads audio from virtio-serial, calls plugin->process()
  └─ writes audio back to virtio-serial
```
...
- `wasivst-worklet.js` — AudioWorkletProcessor. Instantiates QEMU WASM module,
  manages SharedArrayBuffer audio exchange, routes control to virtio-serial.
- `wasivst.js` — Main-thread ES module API (`WasiVST` class). AudioContext
  setup, worklet registration, plugin load, param I/O.

### src/host/

Headless VST2/VST3 host binary for Linux x86-64 (runs under Wine in guest):

- Loads plugin DLL via `LoadLibrary` (Wine/Win32)
- Initializes VST2 `AEffect` or VST3 `IPluginFactory`
- Runs minimal Win32 message pump thread (required by some plugins)
- Reads audio + control frames from virtio-serial stdin, calls plugin
  `processReplacing()` / `IAudioProcessor::process()`, writes output frames
- Serializes with bitsery (reused from src/common/)

### src/guest/

Build scripts for the Linux guest rootfs:

- `buildroot/` — Buildroot external tree (Config.in, external.mk, board config)
- `Dockerfile` — Alternative Alpine-based rootfs builder
- `init` — Minimal /sbin/init: starts wasivst-host, waits for plugin path on
  virtio-serial port 0

### src/common/

Serialization, audio buffer, logging, and plugin ABI types — retained from the
original yabridge codebase. Shared between src/host/ and tooling.

### tools/wasivstctl/

CLI for packaging VST DLLs into `.wasivst` bundles (zip: DLL + metadata JSON),
validating them, and testing locally via headless QEMU.

## Communication Protocol

All host↔worklet communication uses a single virtio-serial byte stream.
Frames are length-prefixed and bitsery-serialized.

| Tag | Direction | Payload |
|-----|-----------|---------|
| LOAD | worklet→host | Plugin path (UTF-8) |
| PROCESS | worklet→host | Sample count, Float32 audio in |
| PROCESS_RESP | host→worklet | Float32 audio out |
| SET_PARAM | worklet→host | Param index (u32), value (f64) |
| GET_PARAM | worklet→host | Param index (u32) |
| GET_PARAM_RESP | host→worklet | value (f64) |
| LOG | host→worklet | Level (u8), subsystem, message |

## QEMU WASM Build

QEMU compiled with Emscripten to wasm32:
- `-s USE_PTHREADS=1` (WASM threads via SharedArrayBuffer + Atomics)
- `-s ALLOW_MEMORY_GROWTH=1`
- TCG backend only (no KVM in browser)
- x86-64 system target (`qemu-system-x86_64`)
- Guest rootfs embedded as data blob or fetched via fetch()

## Observability

`window.__wasivst` live debug registry:

```js
window.__wasivst = {
  logs: [],        // ring buffer, 1000 entries: {ts, level, subsystem, msg}
  instances: {},   // keyed by plugin URL: {state, paramCount, audioState}
  serial: {}       // raw serial channel byte counters
};
```

## Constraints

- No GUI — VST editor windows cannot render in WASM. Audio-only.
- 64-bit plugins only — Wine 64-bit, no 32-bit bitbridge.
- SharedArrayBuffer required — serve with COOP/COEP headers.
- Boot latency ~5-15s — QEMU + Wine startup is not instant.
- Audio latency — virtio-serial adds ~1-5ms IPC overhead per block.
